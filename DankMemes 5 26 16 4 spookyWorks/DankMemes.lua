...
AddDankOption("option_clippy","Clippy xD", "Clippy-chan gives you tips",2);
AddDankOption("option_shieldBreak","HaloShields","Play halo shield noises for shield being broken and for recharging.",2)
AddDankOption("option_fallingMoney","Money Rain","Rain money from the sky when you make bank",4);
AddDankOption("option_spooky","Spooky Scary","When someone casts Fear, it plays 'spooky scary' dank song",4);
...
MoneyFallingImage.DURATION_MAX = DURATION_MAX;
MoneyFallingImage.rotation = 1;
MoneyFallingImage.ROTATION_MAX = 7;
MoneyFallingImage.timer = 0;
MoneyFallingImage.TIMER_MAX = 0.125 * 2;
MoneyFallingImage.duration = 0;
MoneyFallingImage.DURATION_MAX_DEFAULT = 6 * 2;
MoneyFallingImage:Show();
...
--these three are for compatibilty with my new generalized way of doing gifs
...
ClippyImage.setupFunction = function(text)
...
ClippyImage:Show();
end--end function setupFunction
ClippyImage.setupFunction("Hello there. How are you doing today? My name is Clippy and I will be here to annoy you periodically.");
...
if (not(ClippyImage:IsShown())) then return end;
...
if (ClippyImage.textIndex > ClippyImage.textLength + 250/2) then
ClippyImage:Hide();
end
...
if (greenDankZoneSong) then StopSound(greenDankZoneSong) end;
showCryingPepe=true;
